...
@section('content')
    <div class="container">
        ...
        <form class="form-horizontal" method="POST" action="{{ route('vehicleInfo',$vehicles['id']) }}">
            {{ csrf_field() }}
            <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                <label for="name" class="col-md-4 control-label">{{ trans('messages.name') }}</label>

                <div class="col-md-6">
                    <input id="name" type="text" class="form-control" name="name" value="{!! $vehicles->name !!}"
                           required>
                </div>
            </div>
            ...
            <div class="form-group{{ $errors->has('type_id') ? ' has-error' : '' }}">
                <label for="type_id"
                       class="col-md-4 control-label">{{ trans('messages.vehicle type') }}</label>
            ...
            <div class="form-group{{ $errors->has('number_plate') ? ' has-error' : '' }}">
                ...
                <div class="col-md-6">
                    <input id="number_plate" type="text" class="form-control" name="number_plate"
                           value="{!! $vehicles->number_plate !!}" required>
                </div>
            </div>
            ...
            <div class="form-group{{ $errors->has('price') ? ' has-error' : '' }}">
                ...
                <div class="col-md-6">
                    <input id="price" type="text" class="form-control" name="price" value="{!! $vehicles->price !!}"
                           required>
                </div>
            </div>
